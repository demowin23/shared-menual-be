...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    if (!areas) {
      ...
      // Lọc project có areas thuộc danh sách này
      const result = await db.query(
        `SELECT * FROM projects WHERE areas = ANY($1::text[]) ORDER BY created_at DESC`,
        [areaIds]
      );
      projects = result.rows;
    }
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    const project = await db.query('SELECT images FROM projects WHERE id = $1', [id]);
